...
from pixel geometry) and **Signs raster artwork preparation** (§16A;
deliverable: opaque exact-requested-size PNG at human-confirmed physical
width/height, at a target PPI derived purely from that requested size and
this runtime's own proven memory ceiling — never a substrate/product-
category label. Amendment 3.4 (Dimension-Driven Signs Refactor) unified
what amendment 3.3 briefly split into "Rigid Sign"/"Banner" sibling
profiles back into ONE profile: the customer answers only "what size?",
never "what are we making?" — see `resolution-policy.ts`'s own doc for the
exact formula).

**Signs guardrail.** Work on the Signs profile is permitted ONLY inside
explicitly approved Signs phases (S1 and later, each approved by the
product owner). Outside an approved Signs phase, do not add sign
behavior, copy, or claims. This admission authorizes nothing else: no
...
fulfillment, or a customer-facing sign storefront — and no embroidery,
engraving, paper, or 3D engines. The profile admits no bleed, hems,
seams, pole pockets, grommets, wind slits, or other finishing/mounting
geometry (§16A.2, §16A.6) — it is an artwork-preparation admission only,
never a claim about physical substrate or flexibility, and never varies
by one. Constitution §16A.3 (amendment 3.1) admits
