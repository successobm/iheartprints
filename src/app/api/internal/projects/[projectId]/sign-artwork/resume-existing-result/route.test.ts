 ...
 * incident): the operator's "Resume existing provider result" action,
 * through the real route handler. Deliberately narrower in scope than
 * `sign-artwork/prepare/route.test.ts`'s own end-to-end test: THIS route's
 * genuinely new logic is the session gate + precondition-refusal wiring
 * (proven exhaustively here); the capability's own resume/persist/complete
 * behavior is already exhaustively proven at the capability layer in
 * `exhausted-provider-result-recovery.test.ts`, which is where a fake,
 * fully-controllable provider can be injected — the REAL capability graph
 * this route uses resolves its provider from live configuration, so a
 * genuinely successful "attempted" resume is not exercised here (mirrors
 * `prepare/route.test.ts`'s own end-to-end test picking a fixture that
 * needs no provider at all, for the identical reason). Zero Topaz/OpenAI
 * calls — every case here is refused before any provider is ever reached.
 */
describe("POST /api/internal/projects/[projectId]/sign-artwork/resume-existing-result", () => {
  let tempDir = "";
  let previousCwd = "";

  before(() => {
    previousCwd = process.cwd();
    tempDir = mkdtempSync(path.join(tmpdir(), "iheartprints-sign-resume-existing-result-"));
  ...
  it("a job that already completed successfully — refused (status is not failed)", async () => {
    const { graph, repo } = await freshGraph();
    const projectId = (await repo.createProject()).project.id;
    ...
    const body = (await res.json()) as { outcome: string; reason: string };
    assert.equal(body.outcome, "refused");
    assert.equal(body.reason, "job_not_failed");
  });
});
