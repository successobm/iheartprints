/**
 * Phase 27E — GRADUATED FROM EXPERIMENTAL (Phase 27C/27D). This is a
 * byte-for-byte copy of `src/experimental/magic-wand/magic-wand.ts` as it
 * stood at the end of Phase 27D, relocated into the production capability
 * tree so `src/capabilities/artwork-preparation/` does not depend on
 * `src/experimental/` (this codebase's dependency direction keeps
 * experimental code out of the production capability graph). The lab under
 * `src/experimental/magic-wand/` and its standalone `/internal/magic-wand`
 * page are left untouched and keep using their own copy — this file is a
 * deliberate duplication, not a shared import, so neither copy can
 ...
 * invariants below must not change without a demonstrated integration bug
 * (not a UX preference). Do not "improve" this file here — if the algorithm
 * itself needs to change, that is out of scope for graduation work.
 *
