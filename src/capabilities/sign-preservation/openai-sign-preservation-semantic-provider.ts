...
const SYSTEM_INSTRUCTION = [
  ...
  "",
  "For each of the eight required categories, answer exactly one of: same, changed, cannot_determine, not_applicable.",
  "- \"same\": this category is present in the artwork and appears unchanged between the two images.",
  "- \"changed\": this category is present and something about it changed — wording, a number/price, a face, a logo, an object, a meaningful crop, or a perimeter/edge-relative element's alignment to the finished edge.",
  ...
  "",
  "The \"perimeter_edge_alignment\" category is about POSITION, not content: does the artwork contain a border, frame, rounded-corner treatment, mounting-hole/grommet indicator, edge stripe, or any other element whose meaning depends on where it sits relative to the panel's own physical edge? If so, compare where that element sits relative to the RECONSTRUCTED image's own edges against where it sat relative to the ORIGINAL image's own edges. Mark \"changed\" if such an element moved meaningfully inward, outward, or otherwise away from its original edge-relative position (for example: a canvas extension pushed a border or a corner marking away from the edge it used to touch or sit near). Mark \"not_applicable\" only when the artwork genuinely contains no edge-relative element at all — ordinary central content that merely remains visible is NOT enough on its own to mark this category \"changed\"; judge it by position relative to the edge, not by whether central content still looks fine.",
  "",
