...
function buildMessages(
  ...
  const systemPrompt = [
    ...
    'For "product" specifically: look for ANY garment/product noun anywhere in the message, even embedded inside a longer, run-on sentence that is mostly about something else (e.g. team name, occasion, audience). A product noun does not need to be the direct answer to a "what are we printing?" question to count — "help me create a design for team t-shirts" and "we need staff hoodies" both name the product just as concretely as "T-shirts." would. Propose the canonical, customer-facing product name (e.g. "T-shirt", "Hoodie", "Sweatshirt", "Tank top", "Polo", "Jersey", "Cap", "Hat" for common garment synonyms; otherwise the customer\'s own product phrase, cleanly cased) as `value`, and quote the actual phrase the customer used (e.g. "team t-shirts", "staff hoodies") as `evidence` — `evidence` does not need to literally contain the word you chose for `value`, only the customer\'s own product phrase. Use confidence "explicit" whenever a concrete garment/product noun is named, regardless of where in the sentence it appears or what else the sentence is also about.',
