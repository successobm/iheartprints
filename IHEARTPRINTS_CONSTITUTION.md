...
- **Signs raster production** (§16A) — two sibling production profiles sharing one raster-preparation authority: **Rigid Sign** (admitted by amendment 3.0) and **Banner** (admitted by amendment 3.3), each an operator-oriented workflow that turns customer-supplied raster artwork into a validated, exact-size production PNG for its own physical envelope — opaque by default (KEEP), or intentionally transparent when the customer/operator explicitly authorizes background removal (REMOVE, amendment 3.2). Admission is not implementation; §16B records the difference.
...
Production awareness is profile-specific. The facts above are the apparel profile's. For Signs — Rigid Sign or Banner — production awareness centers on ordered physical size, substrate-defined geometry, viewing-distance-appropriate resolution, and preservation of the customer's supplied artwork (§16A), with each profile's own size envelope and resolution figures governing only that profile.
...
Each activated production profile has its own production contract: what it delivers, what it controls, and what its `print_ready` means and does not mean. §16.1–§16.6 are the apparel raster contract, unchanged in substance by amendment 3.0. §16A is the Signs raster contract, covering both the Rigid Sign and Banner sibling profiles. §16B is the profile registry. "V1" in this section refers to the apparel profile's own versioning.
...
| Banner raster | Not apparel — separate activated profile | Admitted by amendment 3.3 as a sibling Signs raster production profile with its own contract (§16A). Never reachable through the apparel contract, and never reachable by treating a large rigid-sign order as a banner — the customer/operator explicitly selects the profile (§16A.1). |
...
## 16A. Signs Raster Production Profiles

### 16A.1 The profiles

Signs Raster is the second activated iHeartPrints production category, admitted as **two sibling profiles** that share one common raster-preparation authority and differ only in their own named, profile-specific facts (physical size envelope and resolution policy):

- **Rigid Sign** (amendment 3.0) — exact-size rectangular rigid signs.
- **Banner** (amendment 3.3) — the same kind of exact-size rectangular raster deliverable, at a larger physical envelope of its own.

Both are **operator-oriented** production-artwork workflows that turn customer-supplied raster artwork into a validated production file. V1 of both is operated by trained operators; neither has a customer-facing self-service *fulfillment* surface, and this admission creates no obligation to build one. (The customer-facing step that exists — explicitly choosing which profile governs an order, before dimensions are asked, §16A.2 — is a routing question, not a fulfillment surface.)

Everything in §16A.2 through §16A.6 below is **shared raster-preparation authority common to both profiles** unless a passage explicitly names one profile's own figures (resolution policy, size envelope) as distinct from the other's. Neither profile may borrow the other's size envelope or resolution figures by default, and an order is never routed between them by inferring one from the other (e.g., "this size is too big for Rigid Sign, so it must be a Banner") — the profile is always an explicit, durable, human-made selection, made before the ordered dimensions are confirmed, and confirmed independently against the selected profile's own envelope. Selecting a profile is production-significant: a subsequent change to the selected profile invalidates any spec, plan, or authorization already confirmed under the prior one, exactly as a change to any other production-significant fact does.

### 16A.2 The deliverable

The Signs deliverable — for either profile — is a **production PNG** that is:
...
- **artwork preparation only, never vendor fulfillment geometry**: the deliverable is the exact-size raster file itself. It carries no bleed, hem, seam, pole-pocket, grommet, or other finishing/mounting geometry unless and until a future amendment deliberately admits that as part of a profile's own contract. A profile whose print-ready raster can be honestly produced without such geometry is not required to invent it.

**Background treatment (amendment 3.2).** Every Signs order, in either profile, carries an explicit, durable background treatment, KEEP or REMOVE:
...
Sign resolution requirements are a **profile policy derived from product class, physical dimensions, viewing distance, print process, and downstream production requirements** — never a universal constant, and never copied from another profile. Each profile's figures below are that profile's own; neither may be applied under the other's name, and neither is extended to any sign class, size, or process beyond what it names.

- **Rigid Sign** (amendment 3.0): rectangular rigid signs up to 24×36 inches — a **150 PPI effective-resolution target with a 100 PPI blocking minimum**.
- **Banner** (amendment 3.3): rectangular banners up to 36×96 inches — a **72 PPI effective-resolution target with a 50 PPI blocking minimum**, deliberately lower than Rigid Sign's for two independently sufficient, converging reasons: (1) Banner's larger physical format is ordinary large-format signage practice for a meaningfully farther typical viewing distance than a near-view rigid sign, and 72 PPI is a legitimate, commonly used large-format target at that distance, not a quality compromise; and (2) this runtime's proven memory ceiling — production/runtime evidence (this build's own DigitalOcean deployment, ~512MB RAM / 1 shared vCPU, and the composition-and-encode pipeline's real, measured peak memory behavior against Banner-sized canvases) shows higher densities leave materially unsafe headroom at Banner's larger physical envelope, and does not authorize approximating Banner's canvas density from Rigid Sign's own figures.

Both are judged from pixel geometry with resolution-provenance honesty: enlarged pixels are judged by their true source detail. Each profile's figures are initial production policy for that profile, expected to evolve from its own production evidence; revising them within a profile is an operational decision recorded in the architecture, not a Constitution amendment. Extending either profile's figures to a different sign class, size, or process — including the other profile — is not authorized by this section.

### 16A.5 What Signs `print_ready` does not mean

`print_ready` under either profile is a statement about the artwork file only. It does not guarantee: substrate quality or suitability, printer calibration, RIP behavior, ICC or color-management accuracy, exact physical color reproduction, mounting, finishing, installation, outdoor longevity, weather resistance, or structural suitability. iHeartPrints does not manufacture, print, ship, mount, or install signs or banners. Print'em All may consume iHeartPrints output and physically fulfill an order; fulfillment remains outside the iHeartPrints product boundary for every profile.

### 16A.6 What this admission does not activate

**Rigid Sign** covers rectangular rigid signs from raster input only. It does not admit: banners or flexible substrates; vehicle wraps; multi-panel murals; contour or cut paths; dimensional letters; installation; bleed or template systems beyond an explicit per-order bleed parameter; SVG/EPS/AI/PDF ingestion; arbitrary vector recreation; general generative redesign, outpainting, or inpainting, or any use of a generative/diffusion model against customer artwork (the sole, narrowly bounded exception — deterministic, non-generative, pixel-sourced perimeter structure reconstruction — is §16A.3's own carve-out, amendment 3.1, not this one); automatic layout redesign; font replacement; CMYK/ICC production workflows; imposition; sign pricing or catalogs; sign fulfillment; or a customer-facing self-service sign storefront.

**Banner** (amendment 3.3) covers exact-size rectangular banner raster preparation, up to its own §16A.4 envelope, from raster input only. Admission is narrowly the same kind as Rigid Sign's own at amendment 3.0 — an artwork-preparation profile, not a claim about physical substrate, material, or flexibility, and not a fulfillment capability. It does not admit: bleed, hems, seams, pole pockets, grommets, wind slits, or any other finishing/mounting geometry (§16A.2); vehicle wraps; multi-panel murals; pole banners or any variant requiring finishing geometry beyond the raster file itself; contour or cut paths; installation; SVG/EPS/AI/PDF ingestion; arbitrary vector recreation; general generative redesign, outpainting, or inpainting, or any use of a generative/diffusion model against customer artwork (governed identically to Rigid Sign — §16A.3's bounded carve-out applies, and nothing wider); automatic layout redesign; font replacement; CMYK/ICC production workflows; imposition; banner pricing or catalogs; banner fulfillment; or a customer-facing self-service banner storefront. Banner's admission never widens, weakens, or reinterprets Rigid Sign's own envelope, resolution policy, or exclusions (§16A.1) — the two remain fully independent profiles sharing only the common raster-preparation authority named in §16A.1.

Each further capability named above, for either profile, requires its own deliberate admission (§7.14). Nor does either profile activate embroidery, engraving, paper-production, or 3D engines — those remain product vision only. Nor does Banner's admission activate any *other* large-format category — vehicle wraps, murals, pole banners requiring finishing geometry, or general commercial printing remain unadmitted (§16, exclusion table) until each is deliberately admitted on its own.
...
| Banner raster | **Admitted** | Admitted by amendment 3.3 as a sibling Signs raster profile (§16A). Implementation proceeds through explicitly approved phases; nothing under this profile is supported, ready, or validated until built and passing its own authoritative validation |
