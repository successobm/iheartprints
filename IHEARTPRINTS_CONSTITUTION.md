...
- **Rigid sign raster production** (§16A) — admitted by amendment 3.0: an operator-oriented workflow that turns customer-supplied raster artwork into a validated, exact-size production PNG for rectangular rigid signs — opaque by default (KEEP), or intentionally transparent when the customer/operator explicitly authorizes background removal (REMOVE, amendment 3.2). Admission is not implementation; §16B records the difference.
...
**Background treatment (amendment 3.2).** Every rigid sign order carries an explicit, durable background treatment, KEEP or REMOVE:
