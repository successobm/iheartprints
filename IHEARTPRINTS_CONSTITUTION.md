...
- **Signs raster artwork preparation** (§16A) — admitted by amendment 3.0: an operator-oriented workflow that turns customer-supplied raster artwork into a validated, exact-requested-physical-size production PNG — opaque by default (KEEP), or intentionally transparent when the customer/operator explicitly authorizes background removal (REMOVE, amendment 3.2). One unified profile, dimension-driven (amendment 3.4): the requested physical size, not a substrate/product-category answer, is what the customer confirms and what the resolution policy is derived from. Admission is not implementation; §16B records the difference.
...
Production awareness is profile-specific. The facts above are the apparel profile's. For Signs, production awareness centers on requested physical size, viewing-distance-appropriate and runtime-safe resolution, and preservation of the customer's supplied artwork (§16A) — never a substrate/product-category answer.
...
Each activated production profile has its own production contract: what it delivers, what it controls, and what its `print_ready` means and does not mean. §16.1–§16.6 are the apparel raster contract, unchanged in substance by amendment 3.0. §16A is the Signs raster artwork-preparation contract, one unified dimension-driven profile. §16B is the profile registry. "V1" in this section refers to the apparel profile's own versioning.
...
| Signs raster artwork preparation | Not apparel — separate activated profile | Admitted by amendment 3.0 (unified onto one dimension-driven policy by amendment 3.4) as its own production profile with its own contract (§16A). Never reachable through the apparel contract. |
| Other large format (vehicle wraps, multi-panel murals, pole banners requiring grommets/finishing), promotional products, general commercial printing | Not apparel | Not admitted (§7.14, §16B, §20). Excluded until deliberately admitted. |

Naming a method as a future capability is a scope statement, never a schedule and never a claim of present support.

## 16A. Signs Raster Artwork Preparation

### 16A.1 The profile

Signs Raster Artwork Preparation is the second activated iHeartPrints production category: an **operator-oriented** production-artwork workflow that turns customer-supplied raster artwork into a validated, exact-requested-physical-size production file. V1 is operated by trained operators; it has no customer-facing self-service *fulfillment* surface, and this admission creates no obligation to build one.

**Dimension-Driven Signs Refactor (amendment 3.4) — the clarified product authority.** Amendment 3.0 admitted this category as a single rigid-sign contract. Amendment 3.3 briefly split it into two sibling profiles — Rigid Sign and Banner — distinguished by an explicit customer "what are we making?" answer asked before dimensions, each with its own size envelope and resolution policy. Real production use proved that split unnecessary: iHeartPrints prepares print-ready artwork; it does not need to know, and no longer asks, what substrate or product the finished sign will later be printed on (ACM, banner vinyl, coroplast, PVC, foam board, or any other Signs substrate are, for artwork preparation, the identical workflow). Amendment 3.4 supersedes amendment 3.3's discrete two-profile split with **one unified Signs raster profile**, governed by a single continuous, dimension-driven policy (§16A.4) rather than a product-category table. The customer answers exactly one question before "Check my artwork": the requested physical width and height. Substrate/product-category is never asked, and is never a decision input anywhere in this profile's preparation, resolution, repair, or validation authority.

This is narrower than it may first read: amendment 3.4 does not admit any NEW substrate, fulfillment capability, or finishing geometry (§16A.6, unchanged in substance). It removes a product-category QUESTION and its associated discrete-table resolution policy, replacing them with a technical, physical-dimension-driven one — the artwork-preparation deliverable, its preservation/repair discipline, its background-treatment and QR-preservation authority, and its validation contract are otherwise identical to what amendment 3.0 (and 3.1, 3.2) already established.

### 16A.2 The deliverable

The Signs deliverable is a **production PNG** that is:

- RGB raster
- sized to the exact requested physical dimensions, width and height, taken from structured order facts a human explicitly confirmed — never inferred from artwork, prose, defaults, or any substrate/product-category answer
- aspect-exact: the plate's pixel geometry matches the requested dimensions, and the customer's artwork within it is never stretched, squashed, or non-uniformly scaled
...
- **artwork preparation only, never vendor fulfillment geometry**: the deliverable is the exact-size raster file itself. It carries no bleed, hem, seam, pole-pocket, grommet, or other finishing/mounting geometry unless and until a future amendment deliberately admits that as part of this profile's own contract. A request whose print-ready raster can be honestly produced without such geometry is not required to invent it.

**Background treatment (amendment 3.2).** Every Signs order carries an explicit, durable background treatment, KEEP or REMOVE:

- **KEEP is the default and the original, unconditional contract.** Unless REMOVE has been explicitly, durably selected, the production plate MUST be fully opaque — no transparency anywhere in the plate — and any transparency is a blocking defect, exactly as this profile has always required.
- **REMOVE is an explicit, human-authorized exception, never inferred.** When durably selected, the plate MAY intentionally carry alpha, produced only through governed background preparation (never a global colour threshold; legitimate same-coloured artwork must be protected by affirmative evidence, not merely colour similarity or enclosure) against the customer's own immutable original. The exact ordered canvas (dimensions, aspect ratio, physical density) is unaffected by transparency — transparent pixels are never cropped, and no canvas dimension may ever be inferred from visible bounds. Accidental or unexplained transparency remains a defect under REMOVE exactly as under KEEP; only transparency produced through the governed path, bound to the current treatment and the current immutable original, is admitted. REMOVE authorizes nothing else in this section or in §16A.6 — in particular, it creates no generative-reconstruction carve-out beyond §16A.3's own bounded provisions.
...
**Bounded perimeter structure reconstruction (amendment 3.1).** A resize to the ordered aspect ratio can require extending the canvas along an axis where the customer's own artwork carries structure whose production meaning depends on the finished physical edge — a border, frame, edge band, a rounded-perimeter treatment, or a simple corner indicator (e.g., a mounting-hole mark) positioned relative to a corner. Extending such an edge with plain background padding is **not** content-preserving repair in this profile: it preserves pixels while silently changing what those pixels mean relative to the finished sign. This amendment admits a narrowly bounded repair category for exactly that case, subject to every constraint below — it is not a general license to redraw, complete, or improve artwork, and every existing constraint in this section still applies in full:

- The reconstructed perimeter must be built **only** from the customer's own already-present pixels — measured, repeated, or geometrically extended patterns already evidenced in the source. It is never generated, inpainted, outpainted, or synthesized by a generative or diffusion model, and never approximates a pattern the system cannot actually measure.
- The **protected interior** — the region diagnosed as central customer content (wording, logos, numerals/prices, faces, meaningful objects, illustrations, and their existing layout relationships) — is never geometrically distorted by this repair beyond uniform proportional scaling. Non-uniform stretch of the interior is prohibited, identically to the general rule above.
- Admission requires affirmative deterministic evidence that the perimeter structure is simple and measurable enough to extend safely — a regular, repeating, or geometrically regular pattern the system can prove, never an inference from an ambiguous or complex composition. Absent that evidence, the system blocks or escalates; it never guesses.
- A corner or hole indicator may be repositioned only when its own structural relationship to a corner is itself simple and unambiguous. This admission never authorizes inferring a manufacturing specification — hole diameter, inset distance, drill placement, corner radius, or any other production/finishing fact — that does not already exist in the artwork or the confirmed order; those remain manufacturing decisions outside the iHeartPrints product boundary (§16A.5), not artwork facts this repair may invent.
- This repair is **always** human-review-required before execution, regardless of what risk classification any other step in the same plan would independently receive; customer authorization alone is never sufficient for it.
- The result is preservation-verified — deterministically for the protected interior and semantically for the reconstructed perimeter's relationship to the finished edge — before any `print_ready` claim, under the same fail-closed discipline as every other reconstruction in this profile.

This amendment authorizes nothing else. General generative redesign, outpainting, inpainting, or any use of a generative/diffusion model against customer artwork remains prohibited by §16A.6, unchanged.

### 16A.4 Resolution policy

Sign resolution requirements are a **profile policy derived from physical dimensions, viewing distance, print process, and this runtime's own proven production capacity** — never a universal constant, and never a product/substrate label. Amendment 3.4 (Dimension-Driven Signs Refactor) replaces the former discrete per-category table (amendments 3.0/3.3: a flat 150/100 PPI rigid-sign figure up to 24×36in, a separate flat 72/50 PPI banner figure up to 36×96in) with one continuous, dimension-driven formula, derived from real measured evidence rather than a second invented table:

- A **memory-safe target**, bounded so the constructed production canvas never exceeds this runtime's own empirically-measured safe pixel budget — exactly the pixel count of an 84×24in canvas at 72 PPI (6,048×1,728px ≈ 10.45 megapixels), the point real, isolated-process measurement of this runtime's actual production pipeline (composition → repair execution → encode, ~512MB RAM / 1 shared vCPU DigitalOcean profile) proved genuinely safe (~257MB peak); the same measurement found ~416MB at 20.2 megapixels (already negligible headroom) and ~566MB at 29.0 megapixels (unsafe).
- A **quality ceiling** of 150 effective PPI — amendment 3.0's own original figure, preserved unchanged as the upper bound, so a smaller sign resolves the identical target it always has (the memory-safe bound only binds once the requested physical area is large enough to need it).
- The **effective target is the lesser of the two** for the requested physical size; the **blocking minimum is two-thirds of the target**, the same ratio amendment 3.0's own 100-of-150 figures already established, applied consistently rather than refit per size class; and the memory-safe ceiling additionally bounds canvas CONSTRUCTION itself (not merely after-the-fact validation), so a customer's own high-resolution artwork can never push a canvas past this runtime's own proven-safe bound, at any requested size — closing a gap the pre-3.4 rigid-sign policy left latent (uncapped canvas construction) simply because its own size ceiling had never been large enough to expose it.
- A request is refused only when even the memory-safe bound would fall below a usable print resolution floor — an honest technical reason, judged from the requested physical size and this runtime's own capacity, never from a product-category boundary. The 40-megapixel/12,000px-per-axis upload INGESTION guard is a separate, unrelated concern (what a customer may upload) and is unaffected by this policy (what the production canvas may become).

This formula is judged from pixel geometry with resolution-provenance honesty: enlarged pixels are judged by their true source detail. It is initial production policy, expected to evolve from production evidence; revising its own figures is an operational decision recorded in the architecture, not a Constitution amendment. It remains a Signs raster policy only — extending it to another production category is not authorized by this section.

### 16A.5 What Signs `print_ready` does not mean

`print_ready` under this profile is a statement about the artwork file only. It does not guarantee: substrate quality or suitability, printer calibration, RIP behavior, ICC or color-management accuracy, exact physical color reproduction, mounting, finishing, installation, outdoor longevity, weather resistance, or structural suitability. iHeartPrints does not manufacture, print, ship, mount, or install signs. Print'em All may consume iHeartPrints output and physically fulfill an order; fulfillment remains outside the iHeartPrints product boundary.

### 16A.6 What this admission does not activate

This profile covers exact-requested-size rectangular Signs raster artwork preparation from raster input only, independent of the substrate the finished piece will later be printed on. It does not admit: vehicle wraps; multi-panel murals; contour or cut paths; dimensional letters; installation; bleed, hems, seams, pole pockets, grommets, wind slits, or any other finishing/mounting geometry beyond an explicit per-order bleed parameter (§16A.2); SVG/EPS/AI/PDF ingestion; arbitrary vector recreation; general generative redesign, outpainting, or inpainting, or any use of a generative/diffusion model against customer artwork (the sole, narrowly bounded exception — deterministic, non-generative, pixel-sourced perimeter structure reconstruction — is §16A.3's own carve-out, amendment 3.1, not this one); automatic layout redesign; font replacement; CMYK/ICC production workflows; imposition; sign pricing or catalogs; sign fulfillment; or a customer-facing self-service sign storefront. Admission is narrowly an artwork-preparation admission — it makes no claim about physical substrate, material, or flexibility, and activates no fulfillment capability for any substrate.

Each further capability named above requires its own deliberate admission (§7.14). Nor does this profile activate embroidery, engraving, paper-production, or 3D engines — those remain product vision only. Nor does it activate any *other* large-format category — vehicle wraps, murals, pole banners requiring finishing geometry, or general commercial printing remain unadmitted (§16, exclusion table) until each is deliberately admitted on its own.
...
| Signs raster artwork preparation | **Admitted** | Admitted by amendment 3.0; unified onto one dimension-driven policy by amendment 3.4 (§16A), superseding the brief two-profile (Rigid Sign/Banner, amendment 3.3) split. Implementation proceeds through explicitly approved phases; nothing under this profile is supported, ready, or validated until built and passing its own authoritative validation |
| Everything else — vehicle wraps, murals, other large-format categories, embroidery deliverables, engraving, vector production, paper/commercial print, 3D, and any category not named above | **Not admitted** | May appear in vision language or dormant architecture; supports nothing; requires its own amendment |
...
Embroidery digitization, screen-print separations, and sublimation-specific preparation are **not** on this list. They are apparel-decoration methods governed by §16.6 — outside current production capability, potentially inside the product later. Absence of capability is not permission to claim support for them today. Signs raster artwork preparation is likewise no longer on this list: it is a single, dimension-driven activated profile governed by §16A, independent of the requesting order's downstream substrate. Vehicle wraps, murals, other large-format categories, and promotional products remain unadmitted.
...
| Version | Summary |
|---|---|
| 3.4 | Dimension-Driven Signs Refactor (sections affected: §16A retitled "Signs Raster Artwork Preparation", §16A.1, §16A.2, §16A.4, §16A.5, §16A.6, §16B, §16 exclusion table, §2/§16/§7 vision language). Reason: amendment 3.3's Rigid Sign/Banner split correctly admitted a genuinely larger physical envelope, but real production use proved the split itself unnecessary — iHeartPrints prepares print-ready artwork; it does not need to know, and should not ask, what substrate the finished sign will later be printed on. A real pending customer order (84×24in) was correctly accepted in size under amendment 3.3 but exposed that the two-profile question itself added no artwork-preparation value it could not get more honestly from the requested physical dimensions alone. Supersedes amendment 3.3's discrete two-policy table with ONE unified, dimension-driven resolution policy (§16A.4): a memory-safe target bounded by this runtime's own empirically-measured safe canvas pixel budget (exactly the previously-validated 84×24in-at-72-PPI figure, 6,048×1,728px), capped above by the original 150 PPI quality ceiling (amendment 3.0, unchanged for smaller signs), with the blocking minimum always two-thirds of the resolved target — replacing two independently-fitted figures with one consistent, evidence-derived formula. Removes the customer-facing "what are we making?" (Rigid Sign/Banner) question entirely (§16A.1) — the requested physical width and height are the only question asked before "Check my artwork" — and removes size envelopes as a rejection reason: a request is refused only when this runtime cannot safely and usefully produce a valid file at the requested size, never for being the "wrong" product label. Reuses the EXISTING intelligent Signs repair/canvas-extension, background-treatment (KEEP/REMOVE, amendment 3.2, unchanged), QR-preservation, and print-ready validation authority entirely unchanged — no new engine, no duplicated repair logic; the decision of whether an order's own artwork can be safely deterministically fit, intelligently extended, or must escalate to human review remains exactly the existing repairability evidence it always was, never a product-category branch. Does not weaken amendment 3.0's original resolution figures for the sizes they already covered safely (a small sign still resolves its full 150 PPI target) and, for the one prior size class where amendment 3.0's own uncapped 150 PPI figure was never actually proven safe against this runtime's real memory ceiling (near amendment 3.0's own 24×36in maximum), corrects it to the same evidence-derived, memory-safe figure every other size now shares — an honest safety correction, not a quality regression invented for this amendment. The former `production_type`/`production_type_confirmed_at` database fields and `SignProductionType` domain vocabulary are retained, unchanged in shape, as inert historical-compatibility metadata only — no migration was required or performed; nothing in this codebase writes a new value to them, and nothing reads them to make a preparation, resolution, or validation decision. Does not admit any new substrate, fulfillment capability, or finishing geometry — §16A.6's exclusions are unchanged in substance, restated for the single unified profile. No change to the apparel contract or to any other admitted profile. |
| 3.3 | Banner Production Profile (sections affected: §16A retitled "Signs Raster Production Profiles", §16A.1–§16A.6, §16B, §16 exclusion table, §2/§16 vision language). Reason: a real customer case — AI-generated artwork for an 84×24in (7ft×2ft) banner — correctly hit §16A.6's exclusion of banners under the Rigid Sign profile, and the honest fix is a genuine sibling production profile, never a raised Rigid Sign ceiling and never an inference that a large sign order is automatically a banner. Admits **Banner** as a second Signs raster production profile, sharing §16A.2–§16A.3 and §16A.5's raster-preparation authority, background-treatment contract (KEEP/REMOVE, amendment 3.2, unchanged), preservation/repair discipline, and print-ready validation architecture with Rigid Sign — none of it duplicated — while defining its own, independently justified facts: a size envelope of rectangular banners up to 36×96in (§16A.4), and its own resolution policy of a 72 PPI effective-resolution target with a 50 PPI blocking minimum — deliberately lower than Rigid Sign's 150/100, derived independently for Banner's own larger physical envelope and typical viewing distance, and separately confirmed safe against this build's own measured runtime memory behavior (not a quality compromise, and not copied from or applied to Rigid Sign). Which profile governs an order is an explicit, durable, human-made selection — asked before ordered dimensions, per §16A.1 — never inferred from size; changing it is production-significant and invalidates any spec, plan, or authorization already confirmed under the prior profile. Banner's admission is narrowly an artwork-preparation admission, identical in kind to Rigid Sign's own at amendment 3.0: it makes no claim about physical substrate, material, or flexibility, and does not admit bleed, hems, seams, pole pockets, grommets, wind slits, or any other finishing/mounting geometry, vehicle wraps, murals, pole banners requiring finishing geometry, banner pricing/catalogs/fulfillment, or a customer-facing self-service banner storefront (§16A.6) — each requires its own deliberate admission. Does not weaken Rigid Sign in any respect: its own envelope, resolution policy (150/100 PPI), and exclusions are unchanged, and neither profile may borrow the other's figures. No change to the apparel contract or to any other admitted profile. |
| 3.2 | Optional rigid-sign background treatment (sections affected: §16A.2). Reason: real production use exposed a valid, narrow need §16A.2's original unconditional-opacity contract did not admit — some sign-adjacent artwork legitimately requires a transparent background (a supplied file with an unwanted exterior background, a clear-substrate or decal use case, downstream production requiring isolated artwork), and no amendment previously distinguished "opaque is the current default" from "opaque is the only possible deliverable." Narrowly amends §16A.2 to admit an explicit, durable, human-selected background treatment — KEEP (default, the original unconditional opaque contract, entirely unchanged) or REMOVE (an explicit exception: the plate may intentionally carry alpha, produced only through governed, non-destructive background preparation reusing the existing apparel/DTF artwork-preparation engine's classification and safety invariant — colour similarity or enclosure alone is never sufficient evidence to remove a pixel — never a second, duplicated removal algorithm, and never a global colour threshold). Does not weaken any other rigid-sign requirement: exact ordered canvas dimensions, aspect ratio, and physical density remain unaffected by transparency; QR/machine-readable-content preservation remains fully authoritative and is made alpha-safe (transparent regions are composited against a fixed neutral analysis background for detection purposes only, never altering the production asset, so hidden colour data beneath a transparent pixel can never influence or forge a machine-readable result); accidental/unexplained transparency remains a defect under REMOVE exactly as under KEEP — only transparency produced through the governed path, bound to the current treatment and the current immutable original, is ever permitted; and `print_ready` still requires every other existing check to pass regardless of treatment. Explicitly creates no generative-reconstruction carve-out: §16A.6's exclusion of general generative redesign, outpainting, and inpainting is unchanged and unweakened — an artwork the removal engine cannot safely classify routes to human review, never a generative guess. No change to the apparel contract, to §16A.3's own bounded perimeter-reconstruction carve-out (amendment 3.1), or to any other section. |
| 3.1 | Bounded perimeter structure reconstruction (sections affected: §16A.3, §16A.6). Reason: real production testing under §16A proved that some sign artwork cannot be made production-ready by pixel-preserving padding alone — a repair can preserve every source pixel, pass every prior automated check, and still change the artwork's meaning by moving a border, frame, or corner indicator away from the finished physical edge it was designed against (the real acceptance case this amendment is written against: a 24×36in warning sign whose perimeter and mounting-hole indicators floated inward under uniform-background padding, machine-validated ready, and was rejected by human production review). Admits one narrowly bounded repair category into §16A.3's existing deterministic-repair-preference bullet: perimeter structure may be reconstructed, but only from the customer's own already-present, measured pixels (never generated, inpainted, outpainted, or synthesized); only outside a diagnosed protected interior, which this repair may never geometrically distort beyond uniform proportional scaling; only on affirmative deterministic evidence the perimeter pattern is simple and measurable, never a guess; never by inferring a manufacturing specification (hole diameter, inset, drill placement, corner radius) absent from the artwork or the order; always human-review-required, with customer authorization alone never sufficient; and preservation-verified, deterministically and semantically, before any `print_ready` claim. §16A.6's exclusion of general generative redesign, outpainting, and inpainting is restated, not weakened — it now names this section's own narrow carve-out explicitly so the two provisions no longer read as silently contradicting each other. No other change to the rigid sign contract, the apparel contract, or any other section. |
